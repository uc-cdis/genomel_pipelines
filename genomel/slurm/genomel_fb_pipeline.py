'''
Main wrapper script for FreeBayes workflow
'''
import os
import time
import argparse
...
def run_pipeline(args, statusclass, metricsclass):
    ...
    jobdir = tempfile.mkdtemp(prefix="fb_%s_" % str(args.output_id), dir=args.basedir)
    ...
    # Setup logger
    log_file = os.path.join(resultdir, "%s.genomel.fb.cwl.log" % str(args.output_id))
    ...
    # Create input json
    input_json_file = os.path.join(resultdir, '{0}.genomel.fb.inputs.json'.format(str(args.output_id)))
    input_json_data = {
      ...
      "output_vcf_name": args.output_id  
    }
...
if __name__ == '__main__':
    ...
    class CallerStatus(postgres.mixins.StatusTypeMixin, postgres.utils.Base):
        __tablename__ = program + '_fb_cwl_status'
    class CallerMetrics(postgres.mixins.MetricsTypeMixin, postgres.utils.Base):
        __tablename__ = program + '_fb_cwl_metrics'
